...
const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbxOGDthyKLSe4-EvfAvjJHjjM-E6mKJY-7U5OgwwswOS1cf-lkBAmXJav80E88h7E5C/exec'
...
function App() {
  ...
  // GoogleスプレッドシートからプロジェクトデータをGAS経由で取得（Axiosに変更）
  const fetchProjects = async () => {
    ...
    try {
      ...
      // axiosはレスポンスを自動的にJSONに変換するので、response.dataで直接アクセス
      if (response.data.status === 'success') {
        setProjects(response.data.data);
        setError(null);
      } else {
        setError(response.data.message || '案件データの取得に失敗しました');
      }
    } catch (err) {
      ...
      if (err.response) {
        ...
        setError(`サーバーエラー: ${err.response.status} ${JSON.stringify(err.response.data)}`);
      } else if (err.request) {
  ...
  const fetchMasterData = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        setOutsources(response.data.data.outsources);
      } else {
        console.error('マスターデータの取得に失敗しました:', response.data.message);
      }
    } catch (err) {
      console.error('マスターデータ取得エラー:', err);
      // エラーを表示するかどうかは要件によります
    }
  };

  // マスターアイテムを追加（Axiosに変更）
  const handleAddMasterItem = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('追加に失敗しました: ' + response.data.message);
      }
    } catch (err) {
  ...
  const fetchOutsourceSummary = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        setOutsourceSummary(response.data.data);
        setShowOutsourceSummary(true);
      } else {
        window.alert('外注別集計の取得に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    } finally {
  ...
  const handleAddProject = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('案件の追加に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
  ...
  const handleDeleteProject = async (id) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('案件の削除に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('ステータスの更新に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
  ...
  const handleOutsourceStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('外注ステータスの更新に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
  ...
  const handleDeliveryDateChange = async (id, newDate) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('納品日の更新に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        window.alert('案件の更新に失敗しました: ' + response.data.message);
      }
    } catch (err) {
      window.alert('サーバーとの通信に失敗しました: ' + err.message);
    }
         ...
         <tbody className="bg-white divide-y divide-gray-200">
           {sortedFilteredProjects.map((project) => (
             <tr key={project.id} className="hover:bg-gray-50">
               ...
               <td className="px-6 py-4 whitespace-nowrap">
                 <div className="text-sm text-gray-900">¥{parseFloat(project.price).toLocaleString()}</div>
               </td>
               <td className="px-6 py-4 whitespace-nowrap">
  {project.outsourced ? (
    <div>
